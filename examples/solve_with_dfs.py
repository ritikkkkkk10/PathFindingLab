"""
Example script demonstrating how to solve a maze using Depth-First Search (DFS).
"""
...
def visualize_solution(maze, solution_path, visited, save_path=None, show=True):
    ...
    plt.title(f"DFS Solution (Path Length: {len(solution_path)}, Cells Explored: {len(visited)})")
...
def main():
    ...
    unsolved_path = os.path.join(OUTPUT_DIRECTORY, "dfs_unsolved.png")
    ...
    if solution_found:
        ...
        solved_path = os.path.join(OUTPUT_DIRECTORY, "dfs_solved.png")
